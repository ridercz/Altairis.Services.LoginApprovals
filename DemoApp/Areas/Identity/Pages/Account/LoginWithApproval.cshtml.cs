        ...
        if (string.IsNullOrEmpty(lasid)) {
            ...
            switch (status) {
                case LoginApprovalSessionStatus.Approved:
                    ...
                    if (user == null) return this.RedirectToPage("LoginWithApprovalFailed");
                ...
                default:
                    ...
                    this.DisplaySessionId = LoginApprovalManager.FormatIdForDisplay(lasid);
                    this.ApprovalUrl = this.Url.Page("Manage/ApproveLogin",
                        pageHandler: null,
                        values: new { lasid },
                        protocol: this.Request.Scheme) ?? throw new Exception("Failed to generate approve login page URL.");
